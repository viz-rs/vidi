...
//!
//! ```
//! # use vidi_core::{IntoResponse, Result};
...
fn generate_handler(input: TokenStream) -> Result<TokenStream> {
        ...
        ast.sig
            ...
            .fold(Vec::new(), |mut extractors, input| {
                if let FnArg::Typed(pat) = input {
                    let ty = &pat.ty;
                    extractors
                        .push(quote!(<#ty as vidi_core::FromRequest>::extract(&mut req).await?));
                }
                extractors
            });
    ...
    let stream = quote! {
        ...
        impl vidi_core::Handler<vidi_core::Request> for #name
        {
            type Output = vidi_core::Result<vidi_core::Response>;

            #[allow(unused, unused_mut)]
            async fn call(&self, mut req: vidi_core::Request) -> Self::Output {
                #ast
                let res = #name(#(#extractors),*)#asyncness;
                #out.map(vidi_core::IntoResponse::into_response)
            }
        }
    };
