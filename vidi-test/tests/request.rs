...
async fn request_body() -> Result<()> {
    ...
    use vidi::middleware::{cookie, limits};
    ...
    let router = Router::new()
        ...
        .get("/cookie", |req: Request| async move {
            Ok(req.cookie("vidi").unwrap().value().to_string())
        })
    ...
    let resp = client
        ...
        .await
        .map_err(Error::boxed)?;
    assert_eq!(resp.text().await.map_err(Error::boxed)?, "viz-rs/vidi");

    let resp = client
        .get("/extract-token")
        .header(AUTHORIZATION, "Bearer vidi")
        .send()
        .await
        .map_err(Error::boxed)?;
    assert_eq!(resp.text().await.map_err(Error::boxed)?, "vidi");

    let mut form = BTreeMap::new();
    form.insert("password", "rs");
    form.insert("username", "vidi");
    let resp = client
        .post("/extract-body")
        .form(&form)
        .send()
    ...
    assert_eq!(
        ...
        r#"{"password":"rs","username":"vidi"}"#
    );

    let resp = client
        .get("/cookie")
        .header(COOKIE, "vidi=crate")
        .send()
        .await
        .map_err(Error::boxed)?;
    ...
    assert_eq!(
        ...
        {
            let mut form = HashMap::new();
            form.insert("username".to_string(), "vidi".to_string());
            form.insert("password".to_string(), "rs".to_string());
            form
        }
    );

    let form = vidi_test::multipart::Form::new()
        .text("key3", "3")
        .text("key4", "4");
    let resp = client
        .post("/multipart")
...
async fn request_session() -> Result<()> {
    use vidi::middleware::{cookie, helper::CookieOptions, session};
    use vidi_test::{TestServer, nano_id, sessions};
